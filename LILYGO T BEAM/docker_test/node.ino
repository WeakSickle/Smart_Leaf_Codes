...
enum STATE
{
  ...
  SENSOR_DATA,
  PMU_INFO,
  DOCKER_TEST, // Lemme just squeeze this in here hehehe
  TRANSMIT
};
...
case GPS_NO_LOCK:
{
  ...
  currentState = DOCKER_TEST; // Move to the next state // SENSOR_DATA
  delay(5000);
  break;
}
case DOCKER_TEST:
{
  ...
  DISPLAY_STATE(); // Display the current state on the screen
  #endif
  Serial.println("Testing Docker ... ");
  delay(5000);
  
  const char* ssid = "your_wifi_name";
  const char* password = "your_wifi_password";

  const char* server = "http://192.168.1.100:8080/api/v1/YOUR_ACCESS_TOKEN/telemetry"; 
