...
float Frequencies[] = {923.2E6, 923.4E6, 923.6E6, 923.8E6, 924.0E6,
                       924.2E6, 924.4E6, 924.6E6, 924.8E6};
...
void loop() {
  for (int i = 0; i < sizeof(Frequencies) / sizeof(Frequencies[0]); i++) {
    ...
    while (millis() - startTime < 1000) {
      // Break if flag is set from setPacketReceivedAction()
      if (receivedFlag) {
        receivedFlag = false;
        Serial.println(F("Packet received!"));
        // Stay on this frequency or exit loop
    ...
    // Stop listening and go to standby (no receive or transmit but keep config)
    radio.standby();
  }
